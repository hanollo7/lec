...
# JAVA_HOME 경로 설정
import os
os.environ['JAVA_HOME'] = 'C:/Program Files/Amazon Corretto/jdk11.0.31_11'
```
...
plt.rc('figure', dpi=120) # 해상도 설정 (dpi)
plt.rc('figure', figsize=(6.5, 6)) # 가로, 세로 크기 설정 (figsize)
...
plt.rc('font', family='NanumGothic') # 폰트 설정 
plt.rc('figure', dpi=120) # 해상도 설정 (dpi)
plt.rc('figure', figsize=(4.33, 4)) # 가로, 세로 크기 설정 (figsize)
sns.barplot(data = top20, y = 'word', x = 'n', hue = 'word')
plt.tight_layout()  #  여백
plt.show()
```
...
#### 1. `wordcloud` 패키지 설치하기
  ...
  `!pip install wordcloud` #앞에서 이미 설치했다면 생략 가능
